...
echo -e "${RED}⚠️  WARNING: This will deploy to MAINNET and use real ETH for gas!${NC}"
echo ""
...
echo "Current ETH balance: $BALANCE ETH"
...
echo "Note: Gas will be paid in ETH"
...
cat > $DEPLOYMENT_FILE << EOF
{
  ...
  "note": "Payment-based reset: Anyone can reset for 1 STRK"
}
EOF
...
echo "  Set (owner):   starkli invoke $CONTRACT_ADDRESS set_counter <VALUE> --network $NETWORK --keystore $KEYSTORE_PATH --account $ACCOUNT_PATH"
echo "  Reset (1 STRK): starkli invoke $CONTRACT_ADDRESS reset --network $NETWORK --keystore $KEYSTORE_PATH --account $ACCOUNT_PATH"
echo ""
