...
mod tests {
    ...
    fn test_contract_read() {
        ...
        dispatcher.set_owner(0.try_into().unwrap());
        assert(0.try_into().unwrap() == dispatcher.owner(), 'Some fuck up happened');
    }
    #[test]
    #[available_gas(200_000_000)]
    #[should_panic]
    fn test_contract_read_fail() {
        let dispatcher = deploy_contract();
        dispatcher.set_owner(1.try_into().unwrap());
        assert(2.try_into().unwrap() == dispatcher.owner(), 'Some fuck up happened');
    }
